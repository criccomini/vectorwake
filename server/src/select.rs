...
fn capped(o: &fleet::Observed) -> bool {
    o.capped && o.rooms.len() as u32 >= o.max_rooms.max(1)
}
...
pub struct Fleet {
    ...
    pub address: String,
    /// The WebTransport door, empty when this instance serves none.
    pub wt: String,
...
pub async fn register_with(
    url: String,
    token: String,
    zone: std::sync::Arc<tokio::sync::Mutex<crate::ArenaServer>>,
) {
    ...
    while let Some(Ok(msg)) = source.next().await {
        ...
        match fleet::tag_of(&data) {
            ...
            Some(fleet::D2A_VIEW) => {
                ...
                // A fresh view is the only moment this process can learn that
                // another opened a room with the same number as one of ours.
                // See ArenaServer::settle_room_numbers for why that is settled
                // here rather than agreed with anybody.
