#!/usr/bin/env python3
# The pilot-and-friends boards, assembled from shared fragments so the chrome
# (top line, rail, row grammar) stays identical across every board. Sizes and
# colors are the drawer's own: 390 wide, ink dfe9f5, dim 6c7a90, friend
# 4fd6ff, key boxes on rgba(63,88,120,.75), lbl 9px mono upper.
import pathlib
...
# The fight behind the drawer, dim under the wash: a wall or two and a hull.
...
def board(body, stops, lit, overlay=""):
...
def frow(name, detail=None, keys="", dim=False, wash=False):
    ...
    return ('<div class="row" style="height:32px;margin:0 -14px;'
            f'padding:0 14px;gap:8px;{w}opacity:{op}">'
...
# The account, reconsidered: not a page but a head. The name large with the
# reroll mark on it, what the name is under it, and the two account acts as
# keys at the far end. Claimed, the keys read password / log out.
def account_head():
    return ('<div class="row" style="height:64px;gap:12px;margin:0 -14px;'
            'padding:0 14px;border-bottom:1px solid rgba(63,88,120,.45)">'
            '<div class="col" style="gap:3px">'
            '<div class="row" style="gap:8px">'
            '<span style="font-size:20px">Delta 154</span>'
            '<svg width="13" height="13" viewBox="0 0 16 16" fill="none" '
            'stroke="#6c7a90" stroke-width="1.3">'
            '<path d="M13 8 A5 5 0 1 1 10.5 3.7 M10.5 1.4 V4 H13.1"/></svg>'
            '</div>'
            '<span class="lbl">a guest on this device</span></div>'
            '<div style="flex:1"></div>'
            + keychip("keep", True) + keychip("log in") + '</div>')

# The friends page's own content, section for section, the way it ships.
def friends_body(trim=False):
    ...
    h += sect("everybody who added you", "4",
              None if trim else "the ones you ignored, and the ones it came"
              " to something")
...
# ---- Current: the pilot page as shipped, for scale ----
...
cur += ('<div style="height:6px"></div>'
        ...
        '<span class="note">a password brings this pilot back anywhere</span>'
        '</div>'
...
write("CurrentPilot.dc.html",
      board(cur, ["play", "ship", "friends", "settings", "pilot"], "pilot"))

# ---- Current: the friends page as shipped, for scale ----
write("CurrentFriends.dc.html",
      board(friends_body(),
            ["play", "ship", "friends", "settings", "pilot"], "friends"))

# ---- Option 1: a friends row that drills into its own drawer ----
o1 = account_head()
o1 += ('<div class="row" style="height:38px;margin:8px -14px 0;'
       'padding:0 14px;gap:8px">'
       '<span style="font-size:16px">Friends</span>'
       '<span class="note" style="margin-left:6px">1 in a game, 1 waiting'
       '</span><div style="flex:1"></div>'
       '<svg width="9" height="12" viewBox="0 0 10 14">'
       '<path d="M2 1.5 L7.5 7 L2 12.5 Z" fill="rgba(79,214,255,.55)"/>'
       '</svg></div>')
write("Option1.dc.html", board(o1, ["play", "ship", "pilot", "settings"],
                               "pilot"))

# ---- Option 1, one level in: the friends drawer, slid in from the right
# the way a slot reading slides over the ship page ----
o1f = ('<div class="row" style="height:36px;gap:10px;margin:2px -14px 0;'
       'padding:0 14px;border-bottom:1px solid rgba(63,88,120,.45)">'
       '<svg width="9" height="12" viewBox="0 0 10 14">'
       '<path d="M8 1.5 L2.5 7 L8 12.5 Z" fill="rgba(79,214,255,.55)"/></svg>'
       '<span style="font-size:14px">friends</span>'
       '<div style="flex:1"></div>'
       '<span class="lbl">swipe right to go back</span></div>')
o1f += friends_body()
write("Option1Friends.dc.html",
      board(o1f, ["play", "ship", "pilot", "settings"], "pilot"))

# ---- Option 2, leading: friends as the body of the same page ----
o2 = account_head() + friends_body()
write("Main.dc.html", board(o2, ["play", "ship", "pilot", "settings"],
                            "pilot"))

# ---- Option 2 with the account as a row section instead of a band ----
o2r = sect("pilot")
o2r += frow("Call sign", "Delta 154, a guest",
            keychip("keep", True) + keychip("log in"))
o2r += friends_body(trim=True)
write("Option2Rows.dc.html", board(o2r, ["play", "ship", "pilot", "settings"],
                                   "pilot"))

# ---- The claim card over option 2, unchanged from today ----
card = ('<div style="position:absolute;inset:0;background:rgba(3,5,10,.55)">'
        ...
        'a password brings Delta 154 back on any machine; without one it '
        'lives on this one until a quiet week reclaims it</div>'
        ...
        + '<span style="width:1px;height:14px;background:#4fd6ff"></span>'
        '</div>'
        '<div class="row" style="gap:8px;margin-top:14px;'
        'justify-content:flex-end">'
        + keychip("cancel") + keychip("set password", True) + '</div></div>')
write("ClaimCard.dc.html", board(o2, ["play", "ship", "pilot", "settings"],
                                 "pilot", overlay=card))
