...
## 58. Hulls are solids, and a bank turns them

**Status:** accepted, extending decision 5 rather than changing it

A hull was its outline triangulated flat, and a bank was `squash = cos(roll)`,
a scale on the drawing's local x. `ship_roll` in `arena.script` has been
working a real lean out of the heading since decision 5, smoothed on a tenth of
a second and clamped at 0.95 radians, and a flat drawing had nowhere to put it:
there is no axis to turn about in two dimensions, so the number came out as a
narrowing.

Each hull is now a solid, built once at load in `arena/hull3d.lua` off the same
outline `world.lua` has always drawn. A deck is lofted over the plan, tall
where the hull is wide and low where it is a wingtip or a nose, because the
height at a point is read off how far inside the outline that point is. Under
it is a keel, a plate about a quarter of the crown's depth. A bank rotates the
whole thing about its own nose-to-tail axis, so a turning ship shows the flank
its thickness gives it instead of getting thinner.

The keel is deliberately not a mirror of the deck. Two matched faces make every
class read the same from either side, which sounds like the top-down argument
and is the opposite of one: a pilot looking at a banked ship should be able to
tell they are seeing its belly. What a mirror was solving, that a hard bank
would otherwise show a face with nothing on it, is solved instead by drawing
the keel's outline as brightly as the deck's. It is the same edge of the same
silhouette, and identity.md puts the class on that edge.

Three things hold it to the rest of the game.

Nothing here moves a vertex in x or y. The plan is the drawing, the drawing is
fitted to the box `sim/src/baseline.c` collides that class in, and all seven
boxes still spend the same 625 square pixels per decision 57. Height is the
only thing invented, which is what lets the whole roster grow a dimension
without any of them growing a hitbox. `hull3d_test.lua` measures every vertex
of every solid against those boxes and `hull_fit_test.lua` holds the drawing to
them from the other side.

The camera does not move. It looks straight down and there is no perspective
lean, because a hull drawn a pixel off where the core collides it is the defect
`hull_fit_test` exists to prevent, and a perspective lean is worst at the edges
of the screen, which is where a wall usually is.

And it stays a drawing. The core still collides the world-axis bounds of an
oriented rectangle in a plane. A rolled hull's box is whatever the core says it
is, exactly as decision 5 says.

**Cost:** geometry. A flat body was fourteen or sixteen triangles and a solid
is thirty-four at rest and seventy holding the whole bank, so both per-frame
layers were resized for a full room of the worst hull the roster can produce.
A layer that runs out does not report it, it stops drawing whatever came last,
so `hull3d_test` prices the room against those figures rather than trusting
them.

**Reconsider if:** the client ever gets a depth buffer for the world layers.
The cull here is a back-face test against a camera that looks straight down,
and it is only safe because everything above the keel is a height field over
the plan and a height field cannot occlude itself. That is also why nothing is
sorted per frame. A depth buffer would retire the argument and allow parts
that overhang.
