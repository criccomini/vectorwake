...
Those are the names the code carried then. The struct that was `Arena` is `Room`
today and the one that was `Zone` is `ArenaServer`, so the vocabulary above
matches this file rather than the source.
