...
nameplate, a side's name on the team list. A reading off a machine is verbatim:
a key cap, a build number, a URL. And the feed sets its lines in lower case with names as the only
