...
[decisions 102 and 143](../architecture/decisions.md).
...
nothing for a key to raise (decision 143).
...
The dial and the board belong to a room you are in, the way the key at the
foot does. The front page has no dial, and a band that opens nothing. A radar
answers what is near you and there is no you out there, and a roster is the
list of a room somebody is standing outside of. The whistle is the same rule,
since the ending is that board with a head over it, so out there a finished
match is a word under the clock and the fight carrying on behind the name.
The strip over the corner divides: the bars are about the connection and the
stands are a live connection, so they stay, while POS goes with the instrument
it is captioned over rather than standing a stranger's tiles under that word.
