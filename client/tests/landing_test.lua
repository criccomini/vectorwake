...
do
    ...
    local pressable = {}
    for _, r in ipairs(ui.hits) do
        if r.action == "land_pick_zone" then pressable[r.value] = true end
    end
    check("and the row for it takes no press",
          pressable.melee and not pressable.war,
    ...
    local row = nil
    for _, r in ipairs(ui.hits) do
        if r.action == "land_pick_zone" and r.value == "melee" then row = r end
    end
...
do
    for _, s in ipairs(SHAPES) do
        ...
        --
        -- Three range rings at this size, which is a dial being read from
        -- across a room rather than off a row.
        check(shape .. " waiting looks for the room it has not found",
              #rings == 3, #rings .. " rings on screen, wanted 3")
        if #rings == 3 and waiting then
            local widest = 0
            for _, g in ipairs(rings) do widest = math.max(widest, g.r) end
