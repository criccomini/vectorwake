...
local function reset(w, h, s)
    ...
    -- What the dial leaves: ui.radar_span() is 162 points on a phone, being
    -- the margin it keeps from the top of the window, the cropped 112 of
    -- dial, the tile readout off its foot and a gap. Written out rather than
    -- imported, because ui.lua drags the whole interface in and this is one
    -- number.
    touch.ceiling = h - 162 * s
    MODS = {}
    LEVEL = {[0] = 0, [1] = 0}
    return w, h, s
end
...
-- dial takes the same 324 pixels off the top whichever way the phone is held,
-- so the room under it is all the orientation changes.
local TIGHT, ROOMY = 456, 1364
