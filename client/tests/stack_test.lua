...
local sim = {
    ...
    -- Trigger 0 is the gun; see TRIG_GUN below.
    ship_level = function(_, t) return (t == 0 and gun_level) or level end,
...
-- --- fragments are the gun's, on a mark that is the bomb's -----------------

-- Shrapnel is bullets. The core reads the rung and the bouncing off the guns
-- at the throw, and the fragment does a bullet of that rung's damage, so the
-- ticks on a bomb mark are drawn from the gun's rung and not from the one the
-- bomb under them is drawn in.
--
-- Both ladders are pulled apart to measure it, because with the hull's two
-- rungs level there is nothing to tell the right answer from the wrong one.
-- This wore the burst's violet, which says a round answered to nobody's aim.
-- True of a charge and never true of a fragment, and the arena has stopped
-- saying it too.
local function frag_ticks(want)
    frame()
    local b = row_box("bomb")
    local left, right, n = math.huge, 0, 0
    for _, sh in ipairs(shapes) do
        local mid = (sh.y0 + sh.y1) / 2
        if b and mid > b.y0 and mid < b.y1 and sh.tint
            and math.abs(sh.tint[1] - want[1]) < 0.002
            and math.abs(sh.tint[2] - want[2]) < 0.002
            and math.abs(sh.tint[3] - want[3]) < 0.002 then
...
-- A bomber on rung 1 whose guns are on rung 3: two fragments, and both of them
-- in the rung the guns reached.
level, gun_level = 1, 3
mods = {[1] = {[3] = 1}}
local gun_hot = pal.hot(pal.rung(3), 0.45)
...
local function frag_span()
    local _, span = frag_ticks(pal.hot(pal.rung(level), 0.45))
    return span
end
mods = {[1] = {[3] = 2}}
