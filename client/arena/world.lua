...
--
-- Hulls chosen for silhouette rather than for meaning. The Apex and the Cipher
-- read as ships at a glance and at any size; the heavier ones are handsome up
-- close and a lozenge at arm's length, which on a title screen is a rock. The
-- Cipher draws itself at 0.72 in an arena on purpose, to be hard to pick out
-- of a fight, and `home_art` multiplies that into whatever a path asks for.
-- The 0.5 below was tuned against a hull that had no such discount, so it is
-- 0.7 now: 0.7 by 0.72 is the half weight the second ship was drawn at.
local HOME_PATHS = {
    {cls = 0, w = 0.20, phase = 0, ox = 0.72, rx = 0.47, ry = 0.75,
     col = "FRIEND", a = 1.0, w1 = 9.0},
    {cls = 4, w = -0.15, phase = 2.6, ox = 0.66, rx = 0.38, ry = 0.55,
     col = "ENEMY", a = 0.7, w1 = 6.0},
}
