...
| `ext/webtransport/` | The browser's QUIC transport, dialled before the socket |
...
Two lines in the whole game are typed into, both on the account card: a call
sign and a password, and only when a player asks to keep a pilot or bring one
onto this device. In a browser the page holds them as real input elements over
the canvas: on a phone because that is the only thing a browser will raise a
keyboard for, and everywhere because a password manager can fill a form and
cannot fill a drawing (decision 37). A native build draws them itself.

An earlier client had an invisible DOM input over the canvas for an address
box, and it produced more defects than any other part of this client: a
keystroke went to whichever of the two held the caret, and after typing, the
canvas never got the keyboard back. What made that unworkable was the
invisibility. Focus had to be moved by hand, on gestures meant for something
else, and handed back afterwards. The elements on the account card are
visible, take their own tap, and exist only while the card is up, so nothing
is traded and there is nothing to hand back.

The address box itself is not coming back. A game whose front door asks for a
websocket URL is a game with a text box in it, and the games list answers the
same question by asking a directory.
...
throw and carried by the bomb, so a pilot on red bullets threw red fragments
and watched violet ones come out. The rung for those cannot come off the spec,
because every fragment in the game is one spec and a rung is a number added to
its damage rather than a row of its own, so `sim.weapon_at` reports the rung
the core wrote on the round and `world.weapons` draws a fragment as the bullet
it is. `tests/round_col_test.lua` compares the two stroke for stroke.
...
Spending the room as it was asked for instead would put a loaded bomb's
fragments through the row above.
...
### The bars come off on the way in

A phone in a browser tab gives the game about four fifths of its screen and
spends the rest on the browser's own furniture, along the edge the thumbs are
already using. Joining a game asks for fullscreen, and on Android that is all
of it back.

Which is worth more here than any offer to install. Adding the page to a home
screen is the other way to a bar-free phone, and it is a decision put to
somebody who came to play a game; fullscreen is the same result with nothing
asked. The install offer stays where it is, in settings, for the people it
suits and for the iPhone, which has no Fullscreen API for a canvas and where
standalone is the only way out of the bars.

Three rules around it, and each is there because of what it stops.

It goes on the join and nowhere else. A fullscreen request with no gesture
behind it is refused, and the tap that picks a game is the gesture, so there is
no other moment available.

It happens once rather than on every join. Chrome posts its own notice about
how to get out every time a page goes fullscreen, so a player working through
four rooms would be told four times. Staying in it across joins costs nothing.

And it never happens again once the player has left. Backing out, swiping the
bars down and turning the setting off all arrive as the same event, and the
page remembers which way the last one went: a game that puts the bars away
again on the next join is arguing with somebody who has already said what they
want.
...
is where a card may be asking to be typed into.
...
Clicking them opens the numbers behind it: frame rate, input margin, estimated
round trip, clock lead, local and remote corrections, visible smoothing debt,
snapshot gaps, bytes each way, tick, and what is in the room.
...
the tools they call. Twice before they were one recipe with a table of numbers
per rung, and both times a player who flew the whole ladder reported the rungs
as one sound. The four bolts took two further attempts after that, both of them
still one instrument at four settings, before landing on phase modulation over a
comb: a burst that starts as a wall of sidebands and clears onto a note, one
note per rung, spelling a descending A minor triad across the ladder. What each
of the eight is meant to be, and the tools that arrived to build them, are in
...
for the detonations. That window is highpassed at 120 Hz first, roughly what a
laptop delivers, because read flat it counts bass nobody can hear and gave the
heaviest bomb the quietest gain in its family.
...
the tinny end of one ladder and the nasty end of the other share an octave, and
what note each bolt lands on, which nothing else there can see: a centroid moves
when anything moves, so it cannot tell a change of pitch from a change of
filter.
