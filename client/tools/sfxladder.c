...
// --- the bolts are tuned ---------------------------------------------------
//
// Every bolt's carrier falls onto a note, and its comb is tuned to the same
// one, so the four of them spell an A minor triad descending across two
// octaves. That is load bearing rather than decorative: intervals the ear
// already knows are the reason a rung can be named on hearing it once instead
// of being compared against its neighbours.
//
// Nothing else in this file would notice a later edit sliding one of them a
// tone off. A centroid moves when anything moves, so it cannot tell a change
// of pitch from a change of filter, and every check above it is built on the
// centroid.
//
// The search is wide on purpose, from well over an octave under the note to
// well over one above. Narrow around the target and a bolt that had lost its
// fundamental and was ringing an octave up would still pass, which is the
// exact failure this caught while the ladder was being built.
static const double GUN_NOTE[4] = {1318.51, 880.00, 523.25, 329.63};
static const char *const GUN_NOTE_NAME[4] = {"E6", "A5", "C5", "E4"};

#define TUNING_CENTS 60.0
#define TUNING_SPAN 2.6
#define TUNING_STEPS 800

// The strongest partial over the back of the sound, where the index has
// collapsed and there is a pitch to find. Goertzel on a log grid: a few
// hundred points against a couple of thousand samples is cheaper to write than
// a transform and runs in nothing.
static double landed_on(const double *x, int n, double target) {
    double lo = target / TUNING_SPAN, hi = target * TUNING_SPAN;
    double best = -1.0, at = 0.0;
    int a = (int)(n * 0.55), k;
    for (k = 0; k < TUNING_STEPS; k++) {
        double f = lo * pow(hi / lo, (double)k / (TUNING_STEPS - 1));
        double w = 2.0 * PI * f / RATE, coeff = 2.0 * cos(w);
        double s1 = 0.0, s2 = 0.0, p;
        int i;
        for (i = a; i < n; i++) {
            double s0 = coeff * s1 - s2 + x[i];
            s2 = s1; s1 = s0;
        }
...
static void check_tuning(void) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        got = landed_on(x, n, GUN_NOTE[i]);
        free(x);
        cents = 1200.0 * log2(got / GUN_NOTE[i]);
        printf("     %-7s lands on %6.1f Hz, wanted %s at %6.1f, %+4.0f cents\n",
               GUN[i], got, GUN_NOTE_NAME[i], GUN_NOTE[i], cents);
        snprintf(line, sizeof(line), "%s lands on %s",
                 GUN[i], GUN_NOTE_NAME[i]);
        check(line, fabs(cents) <= TUNING_CENTS, "it is off its note");
    }
}
